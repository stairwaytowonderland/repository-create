...
async function applySettings(octokit, { owner, repo, settings }) {
...
// src/create-rulesets.ts
async function createRulesets(octokit, { owner, repo, rulesets }) {
  const results = [];
...
async function updateReadmeHeading(octokit, { owner, repo }) {
...
async function fetchReadmeWithRetry(octokit, { owner, repo }) {
...
\u2713 Repository "${repo.full_name}" setup complete.`);
  console.log(`
\u{1F310} Repository available at: ${repo.html_url}`);
  return repo;
}
function createBlank(octokit, { org, name, settings }) {
  return octokit.rest.repos.createInOrg({
     ...
     */
    auto_init: settings.template ? void 0 : settings.auto_init
  });
}
function createFromTemplate(octokit, { org, name, settings }) {
  const { owner: templateOwner, repo: templateRepo, includeAllBranches = false } = settings.template;
